...
//variables to call the information from the keys.js file
...
prompt.get({
	...
	// if the user selects movie it will prompt the user to state what movie they want to look up and then it will get that information from omdb api if the prompt is left blank the function will default and look up Mr Nobody and reutrn that information
		...
		prompt.get({
			...
			if(result.userSelection === ""){
				userSelection = "Lord of the Rings";
			} else{
				userSelection = result.userSelection;
			}
...
function myTwitter(){
	...
	//this sets the variable params to search the username liribot20181 and only return back the last 20 tweets and then it doesn't trim the username so the username information will come up instead of the twitter id#
	var params = {
		screen_name: 'liribot20181',
		count: '20',
		trim_user: false,
	}
